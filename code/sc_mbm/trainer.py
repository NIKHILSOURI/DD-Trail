...
# Replace deprecated torch._six.inf with math.inf for modern PyTorch
inf = float('inf')
...
def train_one_epoch(model, data_loader, optimizer, device, epoch, 
    ...
    total_epochs = getattr(config, 'num_epoch', None) or 1
    batch_iter = data_loader
    if config.local_rank == 0:
        batch_iter = tqdm(data_loader, desc=f'Epoch {epoch+1}/{total_epochs}', unit='batch',
                          leave=False, bar_format='{desc}: {percentage:3.0f}%|{bar}| {n_fmt}/{total_fmt} batches [{elapsed}<{remaining}]')
    for data_iter_step, (data_dcit) in enumerate(batch_iter):
        ...
        if config.local_rank == 0 and hasattr(batch_iter, 'set_postfix'):
            lr = optimizer.param_groups[0]["lr"]
            batch_iter.set_postfix(loss=f'{np.mean(total_loss):.4f}', cor=f'{np.mean(total_cor):.4f}', lr=f'{lr:.2e}')
