...
def pair_wise_score(pred_imgs, gt_imgs, metric, is_sucess):
    ...
    if len(pred_imgs) < 2:
        return [0.0] * len(pred_imgs)  # degenerate: no other samples to compare
    corrects = []
...
def n_way_top_k_acc(pred, class_id, n_way, num_trials=40, top_k=1):
    ...
    for t in range(num_trials):
        ...
        # torchmetrics 0.11+ requires task= and num_classes= for multiclass accuracy
        acc = accuracy(
...
def get_n_way_top_k_acc(pred_imgs, ground_truth, n_way, num_trials, top_k, device, return_std=False):
    ...
    acc_list = []
    std_list = []
    pairs = list(zip(pred_imgs, ground_truth))
    for pred, gt in tqdm(pairs, desc='Top-k class eval', unit='pair', leave=False):
