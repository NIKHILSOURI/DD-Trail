...
# Pillow 10+ removed Image.ANTIALIAS (was LANCZOS).
try:
...
def load_image_data(imagenet_folder, patch_size, *, normalize_to_tanh: bool = False):
    """
    Load ImageNet-Filtered RGB patches.

    normalize_to_tanh:
      False (default): pixels in [0, 1] — legacy behavior for older baseline scripts.
      True: pixels in [-1, 1] via (x*2-1), matching generator tanh outputs for stable D training.
    """
    ...
    for image_folder in os.listdir(imagenet_folder):
        ...
        for image_file in os.listdir(os.path.join(imagenet_folder, image_folder)):
            ...
            img = Image.open(file_path).convert('RGB').resize(patch_size, _RESAMPLE_LANCZOS)
            img_array = np.array(img)
            img_array = img_array / 255.0
            if normalize_to_tanh:
                img_array = img_array.astype(np.float32) * 2.0 - 1.0
            images.append(img_array)
