import os, sys
# Reduce TensorFlow log noise (e.g. from wandb or other deps that load TF)
...
try:
    ...
    import timm.optim.optim_factory as optim_factory  # older timm
import datetime
...
from config import Config_MBM_EEG, _DATA_ROOT
...
def main(config):
    # Ensure single-GPU runs use cuda:0 when launched without torchrun
    if getattr(config, 'local_rank', None) is None:
        config.local_rank = 0
    # print('num of gpu:')
    ...
    if config.local_rank == 0:
        print(f'Using device: {device}' + (f' ({torch.cuda.get_device_name(config.local_rank)})' if device.type == 'cuda' else ''))
    torch.manual_seed(config.seed)
    np.random.seed(config.seed)

    # create dataset and dataloader
    dataset_path = str(Path(_DATA_ROOT) / 'mne_data')
    dataset_pretrain = eeg_pretrain_dataset(path=dataset_path, roi=config.roi, patch_size=config.patch_size,
    ...
    dataloader_eeg = DataLoader(dataset_pretrain, batch_size=config.batch_size, sampler=sampler, 
                shuffle=(sampler is None), pin_memory=(device.type == 'cuda'), num_workers=num_workers,
    ...
    # Use param_groups_weight_decay for newer timm versions (replaces add_weight_decay)
    param_groups = optim_factory.param_groups_weight_decay(model, weight_decay=config.weight_decay)
    ...
    epoch_range = range(start_epoch, config.num_epoch)
    if config.local_rank == 0:
        epoch_range = tqdm(epoch_range, desc='Stage A1 total', unit='epoch', total=config.num_epoch - start_epoch,
                           initial=start_epoch, bar_format='{desc}: {percentage:3.0f}%|{bar}| {n_fmt}/{total_fmt} epochs [{elapsed}<{remaining}, {rate_fmt}]')
    for ep in epoch_range:
        ...
        cor_list.append(cor)
        if config.local_rank == 0 and hasattr(epoch_range, 'set_postfix'):
            epoch_range.set_postfix_str(f'cor={cor:.4f}')
